...
- With =C-x 8= it easy to insert special symbols
...
* EWW browser
- Start with: =eww=, =eww-open-in-new-buffer= or =eww-search-words=.
...
* Claude Code
- Use =C-u= before =claude-code-transient= to pass context.
- Enable ~plan mode~ with double ~shift-tab~: https://claudelog.com/mechanics/plan-mode/
